...
import (
	...
	"github.com/bishopmagnus449/dexter/database"
	"github.com/bishopmagnus449/dexter/log"
)
...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			// handle ip blacklist
			from_ip := req.Header.Get("X-Forwarded-For")
			...
			from_ip2 := req.Header.Get("X-Forwarded-For")
			
			if from_ip2 == "" {
			from_ip2 = req.RemoteAddr
			}

			parts := strings.SplitN(from_ip2, ":", 2)
			remote_addr := parts[0]
